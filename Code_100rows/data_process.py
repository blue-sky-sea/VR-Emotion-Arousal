...
class EEG_AM():
	...
	def get_am_from_eeg(self,filepath):
	    ...
	    #pass
	    return df
	def one_second_NUM_rows(self,df,NUM):
	...
	def am_calculate(self):
		for i in range(len(sr_df)):
			...
			NUM=100
			df=data
			data = self.one_second_NUM_rows(df,NUM)
...
class ECG_HRV():
	...
	def get_freq_from_ecg(self,data,rate,isshow):
	    ...
	    return rv_freq
	def duplicate_df(self,df,times):
	    pd2=pd.DataFrame()
	...
	def get_rv_use_timewindow(self,i):
	    ...
	    delta_millisecond = datetime.timedelta(milliseconds=500)
	    ...
	    dataframe = pd.concat(frames)
	    dataframe.reset_index(drop=True, inplace=True) 
	    dataframe = self.duplicate_df(dataframe,50)
	    return dataframe
